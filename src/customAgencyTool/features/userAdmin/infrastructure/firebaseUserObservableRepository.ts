// Importa las funciones y tipos necesarios de Firebase y RxJS
...
import type { InterfaceCurrentUser } from '../../auth/domain/user.entity';

/**
 * Repositorio para gestionar los datos de 'billsCollections' desde Firestore
 * de forma reactiva usando RxJS y proporcionando paginación.
 */
export class FirebaseUserObservableRepository {
    // --- PROPIEDADES PRIVADAS ---

    private readonly collectionName = 'usersCollection';
    ...
    private dataSubject = new BehaviorSubject<InterfaceCurrentUser[]>([]);
    ...
    public readonly data$: Observable<InterfaceCurrentUser[]> =
        ...
        this.unsubscribeFromSnapshot = onSnapshot(
            ...
            (snapshot) => {
                ...
                const items = snapshot.docs.map((doc) => {
                    ...
                    return result as InterfaceCurrentUser;
                });
                ...
                this.dataSubject.next(FilterPerUser(items));
...
const FilterPerUser = (items: InterfaceCurrentUser[]) => {
    const devEmails = ['greivindaniel1616@gmail.com', 'greivin.castro@uned.cr'];

    const filter = items.filter((item) => {
        if (import.meta.env.VITE_REACT_ENV === 'development') {
            return true;
        } else {
            return !devEmails.includes(item.email);
        }
    });
